 ...
 **/
 function redrawPlayerGalaga(str) {
    ...
    if (luckyLife > 0) {
        ...
        GALAGA_CONTEXT.fillRect(0, 350, Constants.canvasWidth, Constants.canvasHeight); //X, Y, width, height
        GALAGA_CONTEXT.fillStyle = "Black";
        GALAGA_CONTEXT.fillText("GOD MODE!" + luckyLife, 20, Constants.canvasHeight - 30);
    }
    if (isCapturing) {
        tmpYPos -= 2;
    ...
    GALAGA_CONTEXT.fillText("PlayerScore: [" + playerScore + "] Level: [" + level + "] " + str, 20, Constants.canvasHeight - 10);
    for (var i = 1; i < lives; i++) {
        GALAGA_CONTEXT.drawImage(player.img, Constants.canvasWidth - (i * 17), Constants.canvasHeight - 20, 15, 15);
    }
    if (DEBUG) {
